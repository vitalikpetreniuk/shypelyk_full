...
$(function() {
    ...
    shhBody.on('click', '.shh-cart', function(e){
        e.preventDefault();
    ...
    color_picker.find('li').on('click', function(){
        ...
        color_hex = $(this).attr('data-color');
        color_title = $(this).find('.color-title').text();

        $('.mobile-color .color-square').css('background-color', '#'+color_hex);
        $('.mobile-color .color-title').text(color_title);

        $('.item-form .item-color').find('li[data-color="'+color_hex+'"]').trigger('click');
    ...
    $('#country-popup, #postoffice-popup, #delivery-popup, #payment-popup').find('li').on('click', function(){
        $(this).parent().find('li').removeClass('selected');
        $(this).addClass('selected');
    ...
    optionSelect($('#country-popup'), '#shipping_country');
    optionSelect($('#postoffice-popup'), '#shipping_postoffice');
    optionSelect($('#delivery-popup'), '#shipping_place');
    optionSelect($('#payment-popup'), '#payment_type');


    $('.order-promocode .promocode-line').on('click', function(){
        $(this).parent().toggleClass('active');
        $(this).parent().find('promocode-fields').slideToggle();
    })
    ...
    if($('.video').length)
    {
        $('.video-thumbnail').on('click', function(){
            ...
            $(this).parent().find('iframe').prop('src', 'http://www.youtube.com/embed/'+iframe_src+'?autoplay=1');
        })
    }
    ...
    $('select.checkout-select').on('mousedown', function(e) {
        ...
        let $this = $(this);
        let $id = $this.attr('id');
        let payment_popup = $('#payment-popup');
        ...
        switch($id){
            ...
            case 'payment_type':
                payment_popup.addClass('on-screen');

                let payment_options = '';
                payment_popup.find('ul li').each(function(){
                    if(!$this.find('option[value="'+$(this).attr('data-value')+'"]').length)
                    {
                        payment_options += '<option value="'+$(this).attr('data-value')+'">'+$(this).find('span').text()+'</option>';
                    }
                })
                $this.append(payment_options);
        }
    });
    ...
    if(sizeguideForm.length)
    {
        ...
        numInput.on('click tap', function(){
            let $this = $(this);
            ...
            {
                innerLen = $parent.find('input').eq(i).val().length;

                if(innerLen === 0)
                {
                    $this.blur();
                    $parent.find('input').eq(i).focus();
                    break;
                }
            }
        })
